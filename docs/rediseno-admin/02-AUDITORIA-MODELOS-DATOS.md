...
- **`Sucursal.capacidad`** (`{total, auto, moto}`, en null por defecto) y `GET /api/estadias/activas` ahora devuelve un bloque `ocupacion` con `dentro`, `dentroPorTipo`, `capacidad`, `capacidadPorTipo` y `completa`. Con capacidad en null informa el número de adentro **sin denominador**, en vez de inventar un total. Cubierto por cuatro checks en `verify-resolver.js`.
