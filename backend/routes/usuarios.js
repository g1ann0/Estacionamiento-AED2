...
router.get('/', verificarToken, requireRole('admin'), obtenerTodosUsuarios); // Obtener todos los usuarios
router.put('/:dni', verificarToken, requireRole('admin'), actualizarUsuario); // Actualizar usuario

// Obtener datos del usuario específico (debe ir después de rutas específicas)
router.get('/:dni', verificarToken, requireOwnership('dni'), obtenerUsuario);
