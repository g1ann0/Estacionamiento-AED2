...
export const obtenerCuenta = async (dni) =>
  leer(await fetch(`${API}/usuarios/${encodeURIComponent(dni)}`, { credentials: 'include', headers: authHeaders() }), 'No pudimos cargar tu cuenta');
...
export const estadoDeVehiculo = async (dominio) => {
  const res = await fetch(`${API}/estacionamiento/estado/${encodeURIComponent(dominio)}`, { credentials: 'include', headers: authHeaders() });
...
export const misEstadias = async ({ pagina = 1, limite = 20 } = {}) =>
  leer(await fetch(`${API}/estadias/mias?pagina=${pagina}&limite=${limite}`, { credentials: 'include', headers: authHeaders() }), 'No pudimos cargar tu historial');
...
export const descargarMiComprobante = async (id, nombre) => {
  const res = await fetch(`${API}/comprobantes-estadia/${id}/pdf`, { credentials: 'include', headers: authHeaders() });
...
export const miPerfil = async () =>
  leer(await fetch(`${API}/perfil`, { credentials: 'include', headers: authHeaders() }), 'No pudimos cargar tu perfil');

export const actualizarDatosBasicos = async (datos) =>
  leer(await fetch(`${API}/perfil/datos-basicos`, { method: 'PUT', credentials: 'include', headers: authHeaders(), body: JSON.stringify(datos) }), 'No pudimos guardar tus datos');

export const cambiarContrasena = async (datos) =>
  leer(await fetch(`${API}/perfil/cambiar-contrasena`, { method: 'PUT', credentials: 'include', headers: authHeaders(), body: JSON.stringify(datos) }), 'No pudimos cambiar tu contraseña');
